...
const WishlistProduct = (props) => {
  ...
  const handleRemoveFromWishList = (id) => {
    ...
    for (let [key, value] of Object.entries(arr)) {
      if (arr[key].productId == id) removeWishListProduct(arr[key].itemId);
    }
    // debugger
  ...
  return (
    ...
    product.map((p) => {
      // debugger
      return !isEmpty(p.node) && "GroupProduct" !== p.node.type ? (
        <div className="col-lg-4 col-md-6 mb-5">
          ...
          <div className="card-body text-center">
            ...
            {"EXTERNAL" !== p.node.type ? (
              <AddToCartButton product={p.node} />
            ) : (
              <div className="mb-5">
                <a
                  href={p.node.externalUrl}
                  target="_blank"
            ...
            <div className="mb-2" style={{ marginTop: "-10px" }}>
              <button
                onClick={() => {
                  handleRemoveFromWishList(p.node.databaseId);
                }}
